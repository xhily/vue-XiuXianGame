  ...
  <div class="index">
    <div class="index-box">
      ...
      <div class="attributes">
        <div class="attribute-box">
          ...
          <div class="tag attribute" v-if="player.level >= maxLv">
            修为: 登峰造极
          </div>
          <div class="tag attribute" v-else
            @click="gameNotifys({ title: '提示', message: `您距离${levelNames(player.level + 1)}境界还需${formatNumberToChineseUnit(player.maxCultivation - player.cultivation)}点修为` })">
          ...
          <div class="tag attribute">
            闪避率: {{ player.dodge > 0 ? (player.dodge * 100 > 100 ? 100 : (player.dodge * 100).toFixed(2)) :
              0 }}%
          </div>
          <div class="tag attribute">
            暴击率: {{ player.critical > 0 ? (player.critical * 100 > 100 ? 100 : (player.critical *
              100).toFixed(2)) : 0 }}%
          </div>
          <div class="tag attribute">
            总体实力: {{ formatNumberToChineseUnit(player.score) }}
          </div>
          <div class="tag attribute" @click="gameNotifys({ title: '获得方式', message: '每提成一次境界可以获得3点境界点' })">
      ...
      <div class="equip-box">
        <div class="tag equip-item">
          <span class="equip">
            <span>神兵: </span>
            <el-popover v-if="player.equipment?.weapon?.name" placement="bottom" :title="player.equipment?.weapon?.name"
              :width="300" trigger="hover">
              <template #reference>
                <span>
                  <tag :type="player.equipment?.weapon?.quality"
          ...
          <span class="equip">
            <span>护甲: </span>
            <el-popover v-if="player.equipment?.armor?.name" placement="bottom" :title="player.equipment?.armor?.name"
              :width="300" trigger="hover">
              <template #reference>
                <span>
                  <tag :type="player.equipment?.armor?.quality" :closable="player.equipment?.armor?.name ? true : false"
                    @close="equipmentClose('armor')" @click="equipmentInfo(player.equipment['armor']?.id, 'armor')"
        ...
        <div class="tag equip-item">
          <span class="equip">
            <span>灵宝: </span>
            <el-popover v-if="player.equipment?.accessory?.name" placement="bottom"
              :title="player.equipment?.accessory?.name" :width="300" trigger="hover">
              <template #reference>
                <span>
                  <tag :type="player.equipment?.accessory?.quality" :closable="!!player.equipment?.accessory?.name"
                    @close="equipmentClose('accessory')"
                    @click="equipmentInfo(player.equipment['accessory']?.id, 'accessory')"
          ...
          <span class="equip">
            <span>法器: </span>
            <el-popover v-if="player.equipment?.sutra?.name" placement="bottom" :title="player.equipment?.sutra?.name"
              :width="300" trigger="hover">
              <template #reference>
                <span>
                  <tag :type="player.equipment?.sutra?.quality" :closable="!!player.equipment?.sutra?.name"
        ...
        <div class="tag equip-item">
          ...
          <span class="equip">
            <span>灵宠: </span>
            <tag class="pet" v-if="player.pet?.name" :type="computePetsLevel(player.pet?.level)" closable
              @close="petRetract" @click="petItemShow = true">
          ...
          <el-tabs v-model="inventoryActive" :stretch="true">
            <el-tab-pane label="装备" name="equipment">
              <el-dropdown trigger="click" @command="equipmentDropdown" v-if="player.inventory?.length">
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    <el-dropdown-item :command="item.type" v-for="(item, index) in dropdownType" :key="index"
                    ...
                    <div v-if="player.inventory.length">
                      <template v-for="item in player.inventory" :key="item.id">
                        <el-popover placement="bottom" :title="item.name" :width="300" trigger="hover">
                          <template #reference>
                            <span>
                              <tag class="inventory-item" v-if="item.type == i.type" :key="item.id" :type="item.quality"
                                :closable="!item.lock" @close="inventoryClose(item)"
            ...
            <el-tab-pane label="道具" name="props">
              <div class="inventory-content">
                <template v-for="(item, index) in sortedProps" :key="index">
                  <tag type="primary" class="inventory-item"
            ...
            <el-tab-pane label="灵宠" name="pet">
              ...
              <div class="inventory-content">
                <template v-for="(item, index) in player.pets" :key="index">
                  <tag class="inventory-item" :type="computePetsLevel(item.level)" closable @close="petClose(item)"
                ...
                <el-tab-pane :label="i.name" :name="i.type" v-for="(i, k) in player.shopData" :key="k">
                  <div class="inventory-content">
                    <template v-for="(item, index) in i.data">
                      <tag class="inventory-item" :type="item.quality" v-if="item.type == i.type" :key="index"
    ...
    <el-drawer title="修仙境界表" v-model="isLevel" direction="ltr" class="levels">
      <tag class="inventory-item"
        :type="player.level == index ? 'primary' : (index > player.level ? 'danger' : 'success')" :key="index"
      ...
      <div class="monsterinfo">
        <div class="monsterinfo-box">
          <p>
            ...
            <span class="value">
              {{ petInfo.level > parseInt(player.pet?.level || 0) ? levelNames(petInfo.level) :
          ...
          <p>
            <span class="description">闪避率: {{ petInfo?.dodge > 0 ? (petInfo?.dodge * 100 > 100 ? 100 :
              (petInfo?.dodge * 100).toFixed(2)) : 0 }}%</span>
            <span class="icon">
          ...
          <p>
            <span class="description">暴击率: {{ petInfo?.critical > 0 ? (petInfo?.critical * 100 > 100 ? 100 :
              (petInfo?.critical * 100).toFixed(2)) : 0 }}%</span>
            <span class="icon">
              <i :class="calculateDifference(petInfo?.critical, player.pet?.critical).icon" />
            </span>
            <span class="value">{{ calculateDifference(petInfo?.critical, player.pet?.critical).num
            }}</span>
          </p>
          <p>
        ...
        <el-collapse-item name="1">
          ...
          <div class="monsterinfo-box">
            <p>
              ...
              <span class="value">{{ calculateDifference(petInfo?.initial?.health,
                player.pet?.initial?.health).num }}</span>
            </p>
            <p>
              ...
              <span class="value">{{ calculateDifference(petInfo?.initial?.attack,
                player.pet?.initial?.attack).num }}</span>
            </p>
            <p>
              ...
              <span class="value">{{ calculateDifference(petInfo?.initial?.defense,
                player.pet?.initial?.defense).num }}</span>
            </p>
            <p>
              <span class="description">
                闪避率: {{ petInfo?.initial?.dodge > 0 ? (petInfo?.initial?.dodge * 100 > 100 ? 100 :
                  (petInfo?.initial?.dodge * 100).toFixed(2)) : 0 }}%
              </span>
              <span class="icon">
                <i :class="calculateDifference(petInfo?.initial?.dodge, player.pet?.initial?.dodge).icon" />
              </span>
              <span class="value">{{ calculateDifference(petInfo?.initial?.dodge,
                player.pet?.initial?.dodge).num
              }}</span>
            </p>
            <p>
              <span class="description">
                暴击率: {{ petInfo?.initial?.critical > 0 ? (petInfo?.initial?.critical * 100 > 100 ? 100 :
                  (petInfo?.initial?.critical * 100).toFixed(2)) : 0 }}%
              </span>
              <span class="icon">
                <i :class="calculateDifference(petInfo?.initial?.critical, player.pet?.initial?.critical).icon" />
              </span>
              <span class="value">{{ calculateDifference(petInfo?.initial?.critical,
                player.pet?.initial?.critical).num }}</span>
            </p>
          </div>
        </el-collapse-item>
      </el-collapse>
          ...
          <p>
            ...
            <span class="value">
              {{ calculateDifference(levelsNum[inventoryInfo.quality],
                ...
                levels[player.equipment[inventoryInfo.type]?.quality] : levels[inventoryInfo.quality] }} </span>
          </p>
          <p>
    ...
    <el-dialog title="批量处理" :lock-scroll="false" v-model="sellingEquipmentShow" width="600px">
    ...
    <el-dialog v-model="show" :lock-scroll="false" title="游戏设置" width="350px">
      <div class="dialog-footer">
        ...
        <el-upload action="#" class="dialog-upload" :http-request="importData" :show-file-list="false"
        ...
        <el-upload action="#" class="dialog-upload" :before-upload="scriptBeforeUpload" :show-file-list="false"
          accept=".txt,.js">
            ...
            <el-tabs v-model="illustrationsActive" :stretch="true">
              <el-tab-pane :label="i.name" :name="i.type" v-for="(i, k) in illustrationsItems" :key="k">
                <div class="equipAll-content">
                  <template v-for="(item, index) in i.data">
                    <div class="equipAll-item" v-if="item.type == i.type" :key="index"
          ...
          <el-tabs v-model="achievementActive" :stretch="true">
            <el-tab-pane :label="i.name" :name="i.type" v-for="(i, k) in achievementAll" :key="k">
              <div class="achievement-content" v-if="i.data.length > 0">
                <div class="achievement-item" v-for="(item, index) in i.data" :key="index"
    ...
    <el-drawer title="新手礼包" v-model="newBieBox" :before-close="confirmCollectionNewBie" class="newBieBox"
      direction="rtl">
      <div class="newBie">
        <tag v-for="(item, index) in newBieData" class="inventory-item" :type="item.quality" :key="index"
      ...
      <el-button type="primary" :loading="newBieLoading" @click="refreshNewBie">
        {{ newBieLoading ? '刷新中...' : '刷新装备' }}</el-button>
    ...
    <el-dialog v-model="newBieInfoBox" :lock-scroll="false" :title="newBieItem.name" width="420px">
      <div class="monsterinfo">
        <div class="newbieinfo-box">
          ...
          <p>闪避率: {{ newBieItem?.dodge > 0 ? (newBieItem?.dodge * 100 > 100 ? 100 : (newBieItem?.dodge *
            100).toFixed(2)) : 0 }}%</p>
          <p>暴击率: {{ newBieItem?.critical > 0 ? (newBieItem?.critical * 100 > 100 ? 100 : (newBieItem?.critical *
            100).toFixed(2)) : 0 }}%</p>
...
// 监听背包标签页切换
watch(() => inventoryActive.value, (type) => {
...
// 监听玩家属性，防止NaN值
watch(() => player.value.attack, (val) => {
  if (isNaN(val)) reset()
  else return val
})

watch(() => player.value.health, (val) => {
  if (isNaN(val)) reset()
  else return val
})

watch(() => player.value.defense, (val) => {
  if (isNaN(val)) reset()
  else return val
})

watch(() => player.value.maxHealth, (val) => {
  if (isNaN(val)) reset()
  else return val
})

watch(() => player.value.critical, (val) => {
  if (isNaN(val) || val < 0) {
    storyText.value = '暴击属性出错, 请手动卸下身上穿的所有装备后刷新游戏<br>如果身上没有装备却一直存在此提示可以尝试穿一次装备再刷新游戏'
    return 0
  } else {
    return val
  }
...
watch(() => player.value.dodge, (val) => {
  if (isNaN(val) || val < 0) {
    storyText.value = '闪避属性出错, 请手动卸下身上穿的所有装备后刷新游戏<br>如果身上没有装备却一直存在此提示可以尝试穿一次装备再刷新游戏'
    return 0
  } else {
    return val
  }
...
const startGame = () => {
  ...
  actions.value = [
    { text: '开始修炼', handler: () => router.push('/cultivate') },
    {
      ...
      handler: () => {
        if (!player.value.isNewbie) {
          gameNotifys({ title: '提示', message: `新手礼包未领取` })
          return
        }
        if (player.value.level < 10) {
          gameNotifys({
            title: '实力不足提示',
            message: `外面太危险了, 请突破到${levelNames(10)}再出去吧!`,
          })
          return
        }
        router.push('/map')
    ...
    { text: '图鉴与成就', handler: () => { equipAllShow.value = true } },
...
const uploadScript = () => {
  ElMessageBox.confirm('', '脚本导入须知', {
    center: true,
    message:
      '导入前请确认脚本可用并备份存档<br>如因导入错误脚本导致的存档游戏出现任何问题<br>作者概不负责',
...
// 修改名字
const editUserName = () => {
  ElMessageBox.prompt('每次修改名字需要花费100灵石', '修改名字', {
    inputPattern: /^(?=\S).*/,
...
const sellingPet = () => {
  ...
  if (!selling.length) {
    gameNotifys({
      title: '灵宠放生提示',
      message: '你没有可以放生的未锁定灵宠',
    })
    return
  }
  // 关闭弹窗
  sellingEquipmentShow.value = false
  // 计算未锁定灵宠放生所得培养丹
...
const sellingEquipment = () => {
  ...
  // 获取玩家装备分解设置
  const sellingEquipmen = player.value.sellingEquipmentData
  // 检查是否选择了需要分解的品阶
  if (!sellingEquipmen.length) {
    gameNotifys({
      title: '背包装备分解提示',
      message: '你没有选择勾选需要分解的品阶',
    })
    return
  }
  // 过滤出可以分解的装备
  ...
  const strengtheningStoneTotal = selling.reduce((total, i) => {
    let level = i.level + (i.level * player.value.reincarnation) / 10
    level = Number(level) || 0
    return total + Math.floor(level)
  }, 0)
  // 增加灵石数量
...
const equipItem = (id, type) => {
  ...
  if (JSON.stringify(player.value.equipment[type]) != '{}') {
    ...
    playerAttribute(-equipment?.dodge, -equipment?.attack, -equipment?.health, -equipment?.critical, -equipment?.defense)
    // 将当前装备放回背包
  ...
  // 更新玩家属性，添加当前装备的属性加成
  playerAttribute(inventoryItem.dodge, inventoryItem.attack, inventoryItem.health, inventoryItem.critical, inventoryItem.defense)
  // 从背包中移除新装备
...
// 放生灵宠
const petClose = (item) => {
  ElMessageBox.confirm(`你确定要放生<span class="el-tag el-tag--${computePetsLevel(item.level)}">${item.name}(${levelNames(item.level)})</span>吗?`, '灵宠放生通知', {
    center: true,
...
const calculateEnhanceSuccessRate = (item) => {
  ...
  return (
    baseSuccessRate -
    (item.strengthen * decrementPerLevel - (increase.value ? 0.1 : 0))
  )
}
// 灵宠升级
const petUpgrade = (item) => {
  ...
  if (
    petReincarnation.value &&
    player.value.reincarnation < player.value.pet.reincarnation
  ) {
    // 发送通知
    gameNotifys({
      title: '灵宠培养提示',
...
const shopBuy = (item) => {
  if (player.value.props.currency >= shopPrice.value) {
    ...
    // 如果装备背包当前容量大于等于背包总容量
    if (player.value.inventory.length >= player.value.backpackCapacity) storyText.value = `当前装备背包容量已满, 该装备自动丢弃, 转生可增加背包容量`
    // 添加到背包
...
const shopItemInfo = (item) => {
  ElMessageBox.confirm('', item.name, {
    ...
    message: `<div class="monsterinfo">
      ...
      <p>装备评分: ${formatNumberToChineseUnit(item.score)}</p>
      </div>
    </div>`,
...
const newbiePack = (timesLeft) => {
  ...
  // 将装备添加到库存
  if (JSON.stringify(equipItem) != '{}') newBieData.value.push(equipItem)
  // 更新剩余次数
  timesLeft--
  // 设置延时以便下一次调用
  setTimeout(() => {
...
const inventoryClose = (item) => {
  ElMessageBox.confirm(
    `你确定要分解<span class="el-tag el-tag--${item.quality}">${levels[item.quality]
    }${item.name}(${genre[item.type]})</span>吗?`,
    '装备分解通知',
    {
      center: true,
    ...
    const num =
      item.level + Math.floor((item.level * player.value.reincarnation) / 10)
    // 增加炼器石数量
...
const playerAttribute = (dodge = 0, attack = 0, health = 0, critical = 0, defense = 0) => {
  ...
  // 闪避
  player.value.dodge = player.value.dodge + dodge
  // 攻击
  player.value.attack = player.value.attack + attack
  // 血量
  player.value.health = player.value.health + health
  player.value.maxHealth = player.value.maxHealth + health
  // 暴击
  player.value.critical = player.value.critical + critical
  // 防御
  player.value.defense = player.value.defense + defense
  // 评分
...
const equipmentClose = (type) => {
  ...
  // 更新玩家属性，移除当前穿戴装备的属性加成
  playerAttribute(-equipment[type].dodge, -equipment[type].attack, -equipment[type].health, -equipment[type].critical, -equipment[type].defense)
  // 跳转背包相关页
...
const getEquipmentInfo = (id, type) => {
  ...
  // 炼器等级
  if (player.value.equipment[type]) {
  ...
  ElMessageBox.confirm('', info.name, {
    ...
    message: `<div class="monsterinfo">
      <div class="monsterinfo-box">
        ...
        <p>装备评分: ${formatNumberToChineseUnit(info.score)}</p>
        <p>获得率: ${info.prize}%</p>
      </div>
    </div>`,
    cancelButtonText: '关闭',
